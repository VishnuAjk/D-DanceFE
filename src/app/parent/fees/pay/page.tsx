...
import { initiateParentFeePayment } from '@/lib/parent-api';
...
export default function ParentFeePaymentPage() {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const payment = await initiateParentFeePayment({ ledgerIds: selectedIds });
  ...
  return (
    <main className="family-page">
      <section className="admin-page__header">
        ...
        <Link className="button button--ghost" href="/parent/fees">
          Back to fees
        </Link>
      </section>
          ...
          eligibleEntries.map((entry) => {
            ...
            return (
              <button
                ...
                type="button"
              >
                <strong>{child?.name ?? 'Child profile'}</strong>
                <span>
                  {entry.month} • {batch?.name ?? 'Batch'} • {readReferenceLabel(entry.branchId)}
                </span>
                <span>
                  {child ? `DOB ${formatBirthDate(child.dob)} • ${child.gender}` : 'Child profile unavailable'}
                </span>
                <span>
