...
import { createEnrollment } from '@/lib/parent-api';
...
export default function NewEnrollmentPage() {
  ...
  const mutation = useMutation({
    mutationFn: async () => createEnrollment({ childId, batchId }),
    onSuccess: async () => {
    ...
    <main className="family-page">
      <section className="admin-page__header">
        <div>
          ...
          <h1 className="admin-page__title">Choose a branch, batch, and child profile.</h1>
      ...
      <section className="auth-stepper" aria-label="Enrollment progress">
        {[1, 2, 3].map((item) => (
          ...
          >
            ...
            <span>
              {item === 1 ? 'Branch' : item === 2 ? 'Batch' : 'Child'}
            </span>
          </div>
        ))}
      </section>
      ...
      {step === 3 ? (
        <section className="admin-panel stack">
          <label className="field">
            <span className="field__label">Choose child profile</span>
            <select
              className="field__input"
              value={childId}
          ...
          {selectedBatch ? (
            <article className="admin-panel family-summary">
              ...
              <p className="dashboard__text">
                Child: {selectedChild ? selectedChild.name : 'Select a child profile'}
              </p>
            </article>
          ) : null}
          ...
          <div className="admin-panel__actions">
            ...
            <button
              className="button button--primary"
              disabled={!childId || !batchId || mutation.isPending}
