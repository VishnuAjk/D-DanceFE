...
import { apiResponse, batch, student, fulfillJson, ids, instructorUser, mockAuth } from './helpers';
...
test('instructor can mark attendance idempotently for a batch roster', async ({ page }) => {
  ...
  await page.route(`**/api/instructor/batches/${ids.batch}/roster`, (route) =>
    fulfillJson(
      ...
      apiResponse({
        batch: batch(),
        roster: [
          { _id: '64f000000000000000000201', studentProfileId: student(), batchId: ids.batch, status: 'ACTIVE' },
          { _id: '64f000000000000000000202', studentProfileId: secondStudent, batchId: ids.batch, status: 'ACTIVE' }
        ]
      })
    )
  );
  await page.route('**/api/instructor/attendance/mark', async (route) => {
    ...
    const payload = route.request().postDataJSON() as {
      records: Array<{ studentProfileId: string; status: string }>;
    };

    expect(payload.records.map((record) => record.studentProfileId).sort()).toEqual(
      [ids.student, secondStudent._id].sort()
    );
    ...
    await fulfillJson(
      ...
      apiResponse(
        payload.records.map((record) => ({
          _id: `${record.studentProfileId}-${payload.records.length}`,
          batchId: ids.batch,
